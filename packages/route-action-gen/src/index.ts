...
// This script generates the server action files for all the action-config.ts files in the project.
// It searches for the *.action-config.ts files in the project and generates the *.action-creator.ts and *.action.ts files.
// So every time you add a new action-config.ts file or change any existing one, you need to run this script to generate the server action files.
// Usage: npx @workspace/create-server-action
...
const isMainModule = (() => {
  ...
  try {
    ...
    if (
      process.argv[1]?.includes("create-server-action") ||
      process.argv[1]?.includes("index.js") ||
      mainFile.includes("create-server-action") ||
      mainFile.includes("index.js")
    ) {
      return true;
    }
