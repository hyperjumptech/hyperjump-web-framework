...
For example, if you only defined the `bodyValidator`, `searchParamsValidator`, `paramsValidator`, and `auth` function in the config, the `handler` function will be called with an object that contains the `body`, `searchParams`, `params`, and `user` properties.
...
- `AuthFunc`: a generic type that represents the `auth` function.
