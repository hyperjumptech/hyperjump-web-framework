...
- secure: This ensures that the cookie is only sent to the server over HTTPS.
- same-site: This ensures that the cookie is only sent to the server from the same site, which prevents CSRF attacks.
- HTTP-only: This ensures that the cookie is not accessible to JavaScript running in the browser, which prevents XSS attacks.
...
The points to remember when using the HOC are so that the component:
...
Proxy in Next.js, formerly called middleware, is a way to intercept requests and responses. As mentioned in the [official documentation](https://nextjs.org/docs/app/guides/authentication#optimistic-checks-with-proxy-optional), you may optionally run an optimistic check with a proxy.

However we must avoid using proxy if possible for the following reasons:
