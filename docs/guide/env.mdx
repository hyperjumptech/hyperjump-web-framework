...
<Callout type="info" title="Good to know">
  When using Cursor, you can ask the agent to add a new environment variable.
  The agent will use the skill included in this project to add the environment
  variable to the `env.example` file and the `@workspace/env` package. For
  example, you can ask the agent to add two environment variables like this:
  "add NODE_ENV (required) and RESEND_API_KEY (optional)"
</Callout>
