...
4. (Optional) pass `--with-entrypoint` to create an entry point file (`route.ts` for App Router, `index.ts` for Pages Router) if it does not exist. It re-exports from `.generated/`.
