...
When you run `npx route-action-gen`, the CLI generates files in a `.generated/` subdirectory next to each group of config files. It also creates an entry point file in the config directory if one does not exist.
...
For the Next.js Pages Router, the entry point file is `index.ts`:
