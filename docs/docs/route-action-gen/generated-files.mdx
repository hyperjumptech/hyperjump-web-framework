...
When you run `npx route-action-gen`, the CLI generates files in a `.generated/` directory. Entry point files (`route.ts` / `index.ts`) are only created when you pass `--with-entrypoint`.
...
For the Next.js App Router, the entry point file is `route.ts` (created when `--with-entrypoint` is used, or manually):
...
| `use-route-[method].tsx` | A React hook for each HTTP method (e.g. `use-route-get.tsx`, `use-route-post.tsx`). GET hooks return `cancel`, `refetch`, and `lastFetchedAt`; non-GET hooks return `fetchData`. |
...
For the Next.js Pages Router, the entry point file is `index.ts` (created when `--with-entrypoint` is used, or manually). The import path points to the project-root `.generated/` mirror:
...
`use-route-get.tsx` exports `useRouteGet`, which auto-fetches and returns:

| Property        | Type                                        | Description                                                              |
| --------------- | ------------------------------------------- | ------------------------------------------------------------------------ |
| `data`          | `z.infer<typeof responseValidator> \| null` | The validated response data, or `null` before the first successful call. |
| `error`         | `string \| null`                            | Error message from timeout, abort, or failed request.                    |
| `isLoading`     | `boolean`                                   | Whether the request is in flight.                                        |
| `cancel`        | `() => void`                                | Aborts the in-flight request (if any).                                   |
| `refetch`       | `() => void`                                | Triggers a fresh fetch with the same input.                              |
| `lastFetchedAt` | `number \| null`                            | Last refetch trigger timestamp used to drive re-fetching.                |
...
export default function PostPage() {
  const { data, error, isLoading, refetch } = useRouteGet({
    params: { postId: "1" },
  });
...
For non-GET methods (`useRoutePost`, `useRoutePut`, `useRoutePatch`, `useRouteDelete`, etc.), the hook returns `data`, `error`, `isLoading`, and `fetchData`.
