...
| `--with-entrypoint`  |       | `false` | Create missing route entry-point files (`route.ts` / `index.ts`). |
...
6. **Entry point (optional)**: when `--with-entrypoint` is passed, creates `route.ts` (App Router) or `index.ts` (Pages Router) in the config directory if it does not already exist.
