...
If you also want the route entry point created automatically, run:
...
<Files>
  <Folder name="app/api/posts/[postId]" defaultOpen>
    ...
    <File name="route.ts (optional entry point)" />
...
When generated with `--with-entrypoint`, the `route.ts` entry point contains:
