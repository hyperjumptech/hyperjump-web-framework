...
The CLI scans the current directory (recursively) for all `route.[method].config.ts` files and generates code in a `.generated/` subdirectory next to each config file.
