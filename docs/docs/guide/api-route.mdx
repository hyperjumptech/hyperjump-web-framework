...
If you must use route handler, this page provides some rules to follow to keep the code clean and maintainable.
...
- `use-route-[get|post|put|delete|patch|options|head].ts`: A custom hook that can be used to fetch data from the route.
...
- `auth` function: an optional async function that returns a user object if the request is authenticated, otherwise returns null. If you want to reject the request, throw an error.
- `bodyValidator` object that defines the body schema using Zod. Only required for POST/PUT/PATCH requests. The incoming request's body will be validated against this schema.
- `queryValidator` object that defines the query schema using Zod. The incoming request's query/search params will be validated against this schema.
- `paramsValidator` object that defines the params schema using Zod. The incoming request's path params will be validated against this schema. Only valid for route handlers with dynamic segments, e.g., `/api/posts/[postId]/route.ts`.
- `headersValidator` object that defines the headers schema using Zod. The incoming request's headers will be validated against this schema.
- `responseValidator` object that defines the response schema using Zod.
- `handler` function: async function that accepts the validated body, query, params, and headers, and returns a Response object.

### Auth Function

The auth function is the first function to be executed when a request is made to the route. Use this function to check if the request is authenticated.
...
The query validator is a zod object that defines the query schema. It is used to validate the query of the request.
...
`route-next-gen` will generate code to turn the query/search params into an object, then run the validator against the object. If the validation fails, it will return a 400 Bad Request response.

Implementation details based on the example above:

- The value of a query/search param can be a single value or an array of values. In the example above, if the request's search params is `?name=John&filter=active&filter=archived`, the `handler` function will receive the following object: `{ name: 'John', filter: ['active', 'archived'] }`.
...
In the example above, a request to `/api/tags/123/authors/456/views/100` will pass the validation, but a request to `/api/tags/123/authors/456/views/abc` will fail.
...
- `query`: the validated query object if the config defines the `queryValidator`, otherwise it will not be present.
...
For example, if you only defined the `bodyValidator`, `paramsValidator`, and `auth` function in the config, the `handler` function will be called with an object that contains the `body`, `params`, and `user` properties.
...
// TODO: Generate this type automatically if possible
...
}): HandlerResponse<z.infer<typeof responseValidator>>

export const handler: HandlerFunc = async (data) => {
...
In the example above, the config uses three helpers from `@hyperjumptech/route-next-gen/lib`:
...
- `HandlerResponse`: a generic type that represents the successful response from the `handler` function.

These three helpers are needed so that the generated `route.ts` file is also strongly typed.

In the `handler` function, you should catch any errors and return an error response using the `errorResponse` helper. Any uncaught exceptions will be caught by the generated `route.ts` file and will return a generic 500 Internal Server Error response.
...
export const useRoute = ({ options }: { options?: { autoRun?: boolean } }) => {
...
- Create `route-next-gen` tool to generate the code for API routes.
